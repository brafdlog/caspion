...
const startDate = moment().subtract(NUM_DAYS_BACK, 'days').startOf('day').toDate();
  ...
  for(let i = 0; i < accountsToScrape.length; i++) {
    ...
    try {
      ...
      for (let i = 0; i < OUTPUT_VENDORS.length; i++) {
        ...
        const vendorResult = await createTransactionsInVedor(vendor, transactions);
...
async function fetchTransactions(companyId, credentials) {
  ...
  const scrapeResult = await bankScraper.scrape({companyId, credentials, startDate, showBrowser: SHOW_BROWSER });
...
function classifyTransactionCategories(scrapeResult, companyId) {
  ...
  scrapeResult.accounts.forEach(account => {
    const accountTransactions = account.txns.map(txn => ({...txn, companyId, accountNumber: account.accountNumber}));
